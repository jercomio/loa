import type { Loa } from '../../../src/loa.ts'
import loa from '../../../src/loa.ts'
...
function wrapCall<T>(fn: () => T): LoaResult<T> {
  ...
  try {
    ...
    console.log = (...args: unknown[]) => {
      ...
      // @ts-expect-error forwarding unknown[]
      originalLog(...args)
    }
    ...
    const message =
      error instanceof Error ? error.message : 'Unexpected error while calling LOA API'
...
export const loaClient = {
  ...
  splitUUIDAndPrefixed(params: {
    ...
    return wrapCall(() => client.splitUUIDAndPrefixed(params.uuid, params.prefix))
  },
